...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within Leger WebUI for enhanced functionality.

  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run Leger WebUI, ensure connections to localhost use `host.docker.internal`.
...
- Ollama is no longer required to run Leger WebUI.
- Access our comprehensive documentation at [Leger WebUI Documentation](https://docs.openwebui.com/).
